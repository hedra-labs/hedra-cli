...
This CLI is generated by [Fern](https://buildwithfern.com), and the generator vendors
its Rust CLI runtime — **`fern-cli-sdk`** — directly into this repository. That runtime
is the source under [`src/`](src/), built as the `fern_cli_sdk` library target declared
in [`Cargo.toml`](Cargo.toml); it provides the argument binding, HTTP transport,
authentication, output formatting and paging that back every `hedra-cli` command.

`fern-cli-sdk` is licensed under the **Apache License, Version 2.0**. Because its source
is copied into this repository rather than consumed as an external dependency, Apache-2.0
requires that the notice be retained here. The full license text is in
[`LICENSE`](LICENSE) at the repository root, and `hedra-cli` itself is released under the
same license (`license = "Apache-2.0"` in `Cargo.toml`).

You may not use this software except in compliance with the Apache License. Unless
required by applicable law or agreed to in writing, software distributed under the
License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND,
either express or implied. See the [`LICENSE`](LICENSE) file for the specific language
