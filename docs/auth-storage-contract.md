...
| _(unmerged)_ | `src/auth/login.rs` | `fix(sdk): log out of every scheme, and paste into the token binding` — bare `auth logout` clears every declared scheme; `--with-token` pastes into the token binding rather than the OAuth one |

Keep this table current. A patch that is not listed is one nobody will notice
has been reverted.

**Unmerged rows carry a commit subject, not a sha — deliberately.** A sha names
a commit on a branch, so every rebase onto a moved trunk invalidates it,
silently, because a dangling sha looks exactly like a valid one until someone
tries to cherry-pick it. That happened three times in three rebases of the row
above before this note replaced its sha. Find an unmerged patch with
`git log --grep`, and fill in the sha once it has landed on `main` and stopped
moving. Re-read the table after any rebase of an open branch
